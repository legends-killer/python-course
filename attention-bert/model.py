...
class Model(object):
    ...
    def __init__(self, user_count, item_count, cate_count, cate_list, predict_batch_size, predict_ads_num):
        # B是batch大小，T是历史行为的最大值
        self.u = tf.placeholder(tf.int32, [None, ])  # [B]
        self.i = tf.placeholder(tf.int32, [None, ])  # [B]
        self.j = tf.placeholder(tf.int32, [None, ])  # [B]，j是评估和测试的y
        self.y = tf.placeholder(tf.float32, [None, ])  # [B]
        self.hist_i = tf.placeholder(tf.int32, [None, None])  # [B, T]
        self.sl = tf.placeholder(tf.int32, [None, ])  # [B]
        self.lr = tf.placeholder(tf.float64, [])

        hidden_units = 128  # 隐层维度

        # 用hidden_units先嵌入几个特征
        ...
        cate_list = tf.convert_to_tensor(
            cate_list, dtype=tf.int64)  # cate是类别categories
        ...
        hist_i = attention(i_emb, h_emb, self.sl)  # 根据历史行为h是key，计算查询item候选的注意力
        # -- attention end ---
        ...
        hist_j = tf.layers.dense(hist_j, hidden_units,
                                 name='hist_fcn', reuse=True)
        ...
        hist_sub = attention_multi_items(
            item_emb_sub, h_emb, self.sl)  # 对多个item的注意力
        # -- attention end ---

        hist_sub = tf.layers.batch_normalization(
            inputs=hist_sub, name='hist_bn', reuse=tf.AUTO_REUSE)
        # print hist_sub.get_shape().as_list()
        hist_sub = tf.reshape(hist_sub, [-1, hidden_units])
        hist_sub = tf.layers.dense(
            hist_sub, hidden_units, name='hist_fcn', reuse=tf.AUTO_REUSE)
        ...
        din_sub = tf.layers.batch_normalization(
            inputs=din_sub, name='b1', reuse=True)
        d_layer_1_sub = tf.layers.dense(
            din_sub, 80, activation=tf.nn.sigmoid, name='f1', reuse=True)
        #d_layer_1_sub = dice(d_layer_1_sub, name='dice_1_sub')
        d_layer_2_sub = tf.layers.dense(
            d_layer_1_sub, 40, activation=tf.nn.sigmoid, name='f2', reuse=True)
        #d_layer_2_sub = dice(d_layer_2_sub, name='dice_2_sub')
        d_layer_3_sub = tf.layers.dense(
            d_layer_2_sub, 1, activation=None, name='f3', reuse=True)
    ...
    def train(self, sess, uij, l):  # 开始训练
    ...
    def save(self, sess, path):  # 保持模型
        saver = tf.train.Saver()
        saver.save(sess, save_path=path)

    def restore(self, sess, path):  # 重载模型
        saver = tf.train.Saver()
        saver.restore(sess, save_path=path)


def extract_axis_1(data, ind):  # 抽取indices
...
def attention(queries, keys, keys_length):
    ...
    d_layer_1_all = tf.layers.dense(
        din_all, 80, activation=tf.nn.sigmoid, name='f1_att', reuse=tf.AUTO_REUSE)
    d_layer_2_all = tf.layers.dense(
        d_layer_1_all, 40, activation=tf.nn.sigmoid, name='f2_att', reuse=tf.AUTO_REUSE)
    d_layer_3_all = tf.layers.dense(
        d_layer_2_all, 1, activation=None, name='f3_att', reuse=tf.AUTO_REUSE)
    d_layer_3_all = tf.reshape(d_layer_3_all, [-1, 1, tf.shape(keys)[1]])
    outputs = d_layer_3_all
    # 本身行为和最大行为的维度是不一致的，需要mask
    key_masks = tf.sequence_mask(keys_length, tf.shape(keys)[1])   # [B, T]
    key_masks = tf.expand_dims(key_masks, 1)  # [B, 1, T]
    paddings = tf.ones_like(outputs) * (-2 ** 32 + 1)  # 补上极小的值
...
def attention_multi_items(queries, keys, keys_length):
    ...
    # shape : [B, N, T, H]
    queries = tf.reshape(
        queries, [-1, queries_nums, tf.shape(keys)[1], queries_hidden_units])
    ...
    outputs = tf.reshape(
        outputs, [-1, queries_nums, queries_hidden_units])  # [B, N, 1, H]
    print(outputs.get_shape().as_list())
    return outputs
